...
import asset.io.AssetFileGlobber;
...
public class Plugin extends PlayPlugin {
    ...
    public static String addJS(String src, boolean compress) {
        ...
        String result = "";
        for (String fileName : AssetFileGlobber.getResolvedFiles(src, baseUrl)) {
    ...
    private static String getSingleCompressedCSSUrl(String requestKey) {
        return getCompressedUrl("asset.Asset.getSingleCompressedCSS", requestKey);
    }

    private static String getSingleCompressedJSUrl(String requestKey) {
        return getCompressedUrl("asset.Asset.getSingleCompressedJS", requestKey);
    }

    private static String getCompressedCSSUrl(String requestKey) {
        return getCompressedUrl("asset.Asset.getCompressedCSS", requestKey);
    }

    private static String getCompressedJSUrl(String requestKey) {
        return getCompressedUrl("asset.Asset.getCompressedJS", requestKey);
    }
    ...
    private static String getScriptTag(String src) {
        return "<script src=\"" + asset.PluginConfig.contentHostingDomain + src
    ...
    private static String getLinkTag(String src) {
        return "<link href=\"" + asset.PluginConfig.contentHostingDomain + src + "\" rel=\"stylesheet\" type=\"text/css\" charset=\"utf-8\">"
                + (asset.PluginConfig.htmlCompatible ? "" : "</link>") + "\n";
    }
}
