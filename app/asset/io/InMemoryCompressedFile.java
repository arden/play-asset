...
import asset.PluginConfig;
import asset.AssetException;
import asset.AssetLogger;
...
public class InMemoryCompressedFile extends CompressedFile {
    ...
    public boolean exists() {
        ...
        if (bytes != null) {
            ...
            AssetLogger.trace("Got file of size %d bytes from cache in %d milli-seconds.",
    ...
    public InputStream inputStream() {
        if (!exists()) {
            throw new AssetException("Can't create InputStream. File with key " + getCacheKey()
    ...
    public void close() {
        ...
        AssetLogger.trace("Saving file of size %d bytes to cache.", outBytes.length);
    ...
    private void addFileToCache(String filePath, byte[] outBytes) {
        ...
        long totalTime = System.currentTimeMillis() - startTime;
        AssetLogger.trace("Saved file to cache in %d milli-seconds", totalTime);
    }
    ...
    public long length() {
        if (!exists()) {
            throw new AssetException("Can't get length. File with key " + getCacheKey()
                    + " does not exist in cache");
        }
